...
func TestProviderAdapter(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v1/models":
			json.NewEncoder(w).Encode(map[string]interface{}{"data": []map[string]string{{"id": "deepseek-v3"}}})
	...
	resp, err := p.SendChatCompletion(context.Background(), account, &provider.ChatRequest{Model: "deepseek-v3", Messages: []provider.ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
	...
	if !p.SupportsModel("deepseek-r1") {
		t.Fatal("expected deepseek-r1 support")
	}
}
...
func TestProviderErrorNormalization(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "deepseek authentication failed") {
