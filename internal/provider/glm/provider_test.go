...
func TestProviderAdapter(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v1/models":
			json.NewEncoder(w).Encode(map[string]interface{}{"data": []map[string]string{{"id": "glm-5"}}})
	...
	resp, err := p.SendChatCompletion(context.Background(), account, &provider.ChatRequest{Model: "glm-5-apikey", Messages: []provider.ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
	...
	if !p.SupportsModel("glm-4.7") {
		t.Fatal("expected glm-4.7 support")
	}
}
...
func TestProviderErrorNormalization(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "glm authentication failed") {
