...
func TestProviderAdapter(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v1/models":
			json.NewEncoder(w).Encode(map[string]interface{}{"data": []map[string]string{{"id": "minimax-m2.7"}}})
	...
	resp, err := p.SendChatCompletion(context.Background(), account, &provider.ChatRequest{Model: "minimax-m2.7", Messages: []provider.ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
	...
	if !p.SupportsModel("minimax-m2.5-apikey") {
		t.Fatal("expected minimax-m2.5-apikey support")
	}
}

func TestProviderErrorNormalization(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { http.Error(w, "limited", http.StatusTooManyRequests) }))
	defer server.Close()
	err := NewProvider(server.URL).ValidateAccount(context.Background(), &models.Account{Token: "bad"})
	if err == nil || !strings.Contains(err.Error(), "minimax rate limit exceeded") {
