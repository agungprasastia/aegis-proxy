...
func TestProviderAdapter(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v1/models":
			json.NewEncoder(w).Encode(map[string]interface{}{"data": []map[string]string{{"id": "grok-4"}}})
	...
	resp, err := p.SendChatCompletion(context.Background(), account, &provider.ChatRequest{Model: "grok-default", Messages: []provider.ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
	...
	if !p.SupportsModel("grok-default") {
		t.Fatal("expected grok-default support")
	}
}
...
func TestProviderErrorNormalization(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "xai rate limit exceeded") {
